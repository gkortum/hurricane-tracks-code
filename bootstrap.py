...
def dumpBoots(thedf, num_ensembles, num_samples):
    ...
    for m in range(num_samples):
        ...
        changepct, ensembles= theadd_Best(m, thedf, num_ensembles)
        if m < 100:
            hibestens.append(ensembles)
            lowbestens.append(ensembles)
            hichangessofar.append(changepct)
            lowchangessofar.append(changepct)
        else: 
            
            hibestens = [x for _,x in sorted(zip(hichangessofar,hibestens))]
            hichangessofar.sort()
            lowbestens = [x for _,x in sorted(zip(lowchangessofar,lowbestens))]
            lowchangessofar.sort()
            
            
            lowest_hichangesofar = hichangessofar[0]
            highest_lowchangesofar = lowchangessofar[-1]
            
            
            if changepct>lowest_hichangesofar: 
                hichangessofar[0]= changepct
                hibestens[0] = ensembles
                
            elif changepct < highest_lowchangesofar: 
                lowchangessofar[-1] = changepct
                lowbestens[-1] = ensembles
    for i in range(0,100,1):            
        with open('data/bootstrap ensembles 2/lowbestens_'+str(i), 'wb') as f:
            pickle.dump(lowbestens[i],f)

        with open('data/bootstrap ensembles 2/hibestens_'+str(i), 'wb') as f:
            pickle.dump(hibestens[i],f)
        
    with open('data/bootstrap ensembles 2/lowchanges', 'wb') as f:
        pickle.dump(lowchangessofar,f)

    with open('data/bootstrap ensembles 2/hichanges', 'wb') as f:
        pickle.dump(hichangessofar,f)
#     hichangessofar.append(hichangesofar)
#     lowchangessofar.append(lowchangesofar)
